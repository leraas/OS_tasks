...
Matrix createMatrix(int rows, int cols) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            matrix.data[i][j] = 1.0 + ii; // Ініціалізація до нуля
            ii++;
        }
    }
    return matrix;
...
int main() {
    ...
    Matrix scaledMatrix = multiplyMatrixByScalar(matrix1, 3.0);
    printf("\nMatrix 1 multiplied by 3.0:\n");
    ...
    printf("Element (1,1) of matrix 1: %.2f\n", getElement(matrix1, 1, 1));
